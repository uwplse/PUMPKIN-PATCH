...
When hacking on Pumpkin, we recommend using a Coq installation from OPAM, so that you can easily browse the source and get code completion with Merlin (used by most editors' OCaml plugins). The following steps will reinstall Coq from OPAM while saving the source and symbol/type information in a format usable by Merlin:
```shell
$ export OPAMKEEPBUILDDIR=true
$ export OCAMLPARAM="_,bin-annot=1"
$ opam reinstall coq.8.6 # or just "install", if Coq was not previously installed
$ make merlin # to generate the .merlin config file
```
