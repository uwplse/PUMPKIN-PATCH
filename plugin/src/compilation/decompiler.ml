...
(* Abstract over if/else chains when creating tactics.
   Returns default if nothing could be reasonably decompiled. *)
let try_default default b (f : unit -> tact list) : tact list =
  if b then f () else default

(* A proper rewrite must have 6 arguments and be one of the
   common eliminators for equality generated by coq. *)
let try_rewrite def f args rewr =
  try_default def (Array.length args == 6 && is_rewrite f)
    (fun () ->
...
let tac_from_term env trm : tact list =
  let rec first_pass env trm =
    ...
    let try_tac = try_default default in
    match kind trm with
    (* "fun x => ..." -> "intro x." *)
    | Lambda (n, t, b) ->
    ...
    | App (f, args) ->
       try_rewrite default f args (fun left prf hyp ->
       ...
       (match kind valu with
        | App (f, args) ->
           ...
           try_rewrite default f args (fun left prf hyp ->
               ...
               try_tac (isRel hyp) (fun () ->
                   let idx = destRel hyp in
                   (* Old hypothesis is never referenced again. *)
                   try_tac (Vars.noccurn (idx + 1) body) (fun () -> 
