...
(* Guard fails if the rel index occurs in the term. *)
let noccur (i : int) (trm : constr) : unit option =
  if Vars.noccurn i trm then Some () else None
...
let rec first_pass env trm =
  ...
  let choose f x = Option.default [Apply (env, trm)] (f x env) in
  match kind trm with
  (* "fun x => ..." -> "intro x." *)
  | Lambda (n, t, b) ->
...
and rewrite (f, args) env : tact list option =
  try_rewrite f args >>= fun (left, prf, hyp) ->
  Some (Rewrite (env, prf, left) :: first_pass env hyp)

and left (f, args) env : tact list option =
  try_fun f args or_introl 3 >>= fun args ->
  Some (Left :: first_pass env (Array.get args 2))

and right (f, args) env : tact list option =
  try_fun f args or_intror 3 >>= fun args ->
  Some (Right :: first_pass env (Array.get args 2))

and split (f, args) env : tact list option =
  try_fun f args conj 4 >>= fun args ->
  let lhs = first_pass env (Array.get args 2) in
...
let tac_from_term env trm : tact list =
  ...
  reflexivity (collapse_intros (first_pass env trm))
