...
let rec args_to (env : env) (f : types) (trm : types) : env * (types array) =
  ...
  | App (g, args) ->
     if convertible env g f then
       ...
       let envs_args = List.map (args_to env f) (Array.to_list args) in
...
let update_theorem (env : env) (src : types) (dst : types) (trm : types) : types =
  ...
  let patch = all_conv_substs env (src, dst) trm in
  let patch_dep =
    if num_src_hs = num_dst_hs then
      let patch = shift_by num_src_hs patch in
      unshift_by num_src_hs (all_conv_substs env_s (src_concl, dst_concl) patch)
    else
      patch
  in
  let patch_lambda = reconstruct_lambda env patch_dep in
  try
    let _ = infer_type env patch_lambda in
    patch_lambda
  with _ ->
